  ...
  return (
    <View style={s.wrap}>
      ...
      <Text style={s.updated}>{
        lang === 'en' ? `Updated ${dd}/${mm}/${yyyy} ${hh}:${mi}`
        : lang === 'fa' ? `به‌روزرسانی ${dd}/${mm}/${yyyy} ساعت ${hh}:${mi}`
        : lang === 'ru' ? `Обновлено ${dd}/${mm}/${yyyy} ${hh}:${mi}`
        : `מעודכן ליום ${dd}/${mm}/${yyyy} שעה ${hh}:${mi}`
      }</Text>
      <View style={s.row}>
        {items.map((it, i) => (
...
const s = StyleSheet.create({
  ...
  card: { flex: 1, backgroundColor: Colors.WHITE, borderRadius: 10, paddingHorizontal: 4, paddingVertical: 8, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 1 },
  flag: { fontSize: 16 },
  value: { fontSize: 12, fontWeight: '900', color: Colors.PRIMARY, marginTop: 2 },
  label: { fontSize: 8, color: '#888', marginTop: 1 },
});
