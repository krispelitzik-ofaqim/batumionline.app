...
export const markSubmitted = async (phone: string) => {
  try { await AsyncStorage.setItem(MY_SUBMISSION_FLAG, phone || '1'); } catch {}
};
...
export default function DeleteMySubmission({ accentColor = Colors.PRIMARY }: { accentColor?: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const v = await AsyncStorage.getItem(MY_SUBMISSION_FLAG);
        setHasFlag(!!v);
        if (v && v !== '1') setPhone(v);
      } catch { setHasFlag(false); }
    })();
  }, []);
  ...
  const del = async (id: string) => {
    ...
    try {
      ...
      if (j.success) {
        ...
        setItems(next);
        if (next.length === 0) {
          await AsyncStorage.removeItem(MY_SUBMISSION_FLAG);
          setHasFlag(false);
          setOpen(false);
        }
        Alert.alert('נמחק');
